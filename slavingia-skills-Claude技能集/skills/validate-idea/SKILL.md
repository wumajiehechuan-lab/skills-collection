...
- **前置条件：** 已通过 ind-community 明确了目标社区和至少一个潜在痛点。
- **下一步：** 验证通过后 → processize（流程化，手动交付）
- **如果验证不通过：** 回到 ind-community 重新寻找社区和痛点。
...
验证通过后，引导用户进入下一步：
"想法验证通过了。现在不要急着写代码——我们先手动交付，看看能不能真的帮到人。使用 processize 来设计你的手动交付流程。"
