...
**这是极简创业九步流程的第 3b 步（构建阶段的后半部分）。**

- **前置条件：** 已通过 processize 跑通了手动流程，有付费客户，流程文档稳定。
- **下一步：** MVP 就绪后 → irst-customers（首批客户）
- **与流程化的关系：** 流程化在前，MVP 在后。先手动交付、流程文档化，再产品化。
...
- **Charge money.** There's a huge difference between free and  (the zero price effect). Charge something.
...
MVP 就绪后，引导用户进入下一步：
"MVP 就绪。现在最关键的：一个一个地去卖。使用 irst-customers 来制定你的首批客户获取策略。"
